...
-(instancetype)initWithViewController:(UIViewController *)webVC configuration:(WKWebViewConfiguration *)configuration {
    ...
    if (self) {
        ...
        [configuration.userContentController addScriptMessageHandler:self name:@"testCallJs"];
    }
    return self;
}
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([message.name isEqualToString:@"showVideo"]) {
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:message.name]];
...
-(void)cancelHandler {
    ...
    [_configuration.userContentController removeScriptMessageHandlerForName:@"testCallJs"];
}
